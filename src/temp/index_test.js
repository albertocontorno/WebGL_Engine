...
var scene = new Scene();
const req = new Request('http://localhost:1234/assets/Rivet Gun.obj');
const reqMtl = new Request('http://localhost:1234/assets/rivetGun_low.mtl');
...
Promise.all([req.send(), reqMtl.send()]).then( res => {
  ...
  console.log(mtl);
  o.onUpdate = () => {
     o.transform.rotation[1] += 10 * engine.time.deltaTime; 
  }
...
dirLight.ambient = vec3(1, 1, 1);
dirLight.diffuse = vec3(1.0, 1.0, 1.0);
dirLight.direction = vec3(0.5, -1.0, 0.1);
