...
Promise.all([req.send(), reqMtl.send()]).then( res => {
  ...
  m.geometries.forEach( g => {
    ...
    if(ooMat['diffuseMap']){
      let diffuseMap = new Texture(2, null, 'assets/'+ooMat['diffuseMap'], gl.RGB, gl.RGB, {flipY: true});
      diffuseMap.LoadTexture(gl);
      ooMesh.textures.DIFFUSE_MAP = diffuseMap;
    }
    if(ooMat['normalMap']){
      console.log('NORMAL MAP ' + ooMat['normalMap'])
      let normalMap = new Texture(0, null, 'assets/'+ooMat['normalMap'], gl.RGB, gl.RGB, {flipY: true});
